...
async function runCaptureAI() {
  ...
  try {
    ...
    const p = await _groqVisionJson(captureImageDataUrl, {
      systemPrompt: sysP,
      userPrompt: usrP,
    ...
    if (p.name)  { document.getElementById('cap-name').value = toTitleCase(_cleanAiName(p.name)); flash('cap-name'); }
    captureDescription = p.description ? formatDescription(p.description) : '';
    ...
    const filled = [p.name ? 'nombre' : null, captureDescription ? 'descripción' : null,
                    ...
                    catSet ? 'categoría' : 'sin categoría — quedó en "Por revisar"'].filter(Boolean);
...
function updateCapSaveBtn() {
  ...
  if (!hint) return;
  hint.innerHTML = ok ? '' : '<svg width="13" height="13" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:3px"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>Falta el nombre del producto';
}
