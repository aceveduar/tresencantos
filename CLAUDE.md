...
- **Efectivo rápido dinámico según el total (2026-08-21)** — los 3 botones de efectivo rápido eran fijos ($100/$200/$500) sin importar el total de la venta: para una compra de $3,304, ninguno de los tres alcanzaba a cubrirla, así que ninguno era realmente "rápido" (solo servía "Exacto"). `_posQuickCashAmounts(total)` (`pos-cart.js`) calcula, sobre el total con descuento (`getDiscountedTotal()`), el siguiente monto ≥ total redondeando a denominaciones reales de billete en México (100/500/1000/2000/5000), hasta 3 opciones únicas — para $3,304 ahora ofrece $3,400/$3,500/$4,000 en vez de $100/$200/$500. `_updateQuickCashButtons()` reescribe el texto y el `onclick` de `#quick-cash-1/2/3` (antes botones con montos fijos en el HTML) cada vez que cambia el total o el descuento — se llama desde `updateChange()`, que ya corría en todos esos casos. Si el total cae por debajo de 3 opciones útiles, el botón sobrante se oculta en vez de repetir un monto. El bloque de anticipo de apartados (25%/50%/75%/Exacto) es un patrón distinto (porcentajes del total, no montos de efectivo) y no se tocó. CACHE_VERSION v279→v280.
