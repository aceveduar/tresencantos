...
- **Escanear código de barras** (Quagga2, todos los dispositivos) en campo barcode, captura rápida, búsqueda, kit builder y recepción — solo formatos de barras (EAN-13/8, UPC-A/E, Code-128), sin QR
...
- **Escáner Quagga2 — lecturas erróneas y enfoque lento** — tras el cambio a Quagga2 universal, prueba real en Android: el escáner detectaba códigos pero confundía dígitos y tardaba en enfocar. Corregido en `admin-scanner.js` y `pos-checkout.js` con tres ajustes en `Quagga.init()`/`onDetected`: (1) `inputStream.constraints` ahora pide `width`/`height` ideal 1280×720 (mín. 640×480) y `advanced:[{focusMode:'continuous'}]` — mejor resolución y enfoque continuo si el dispositivo lo soporta (constraint best-effort, se ignora si no); (2) se descarta cualquier lectura cuyo error promedio de `decodedCodes[].error` supere `0.15` (`_QUAGGA_MAX_ERROR`/`_POS_QUAGGA_MAX_ERROR`); (3) se exige que el mismo código se lea 2 veces seguidas (`_QUAGGA_CONFIRM`/`_POS_QUAGGA_CONFIRM`) antes de aceptarlo y despachar `_onAdminScan()` / agregar al carrito — filtra confusiones de un solo frame. Pendiente de confirmar mejora en dispositivo real. (2026-06-11)
...
- **Escáner de código de barras — Quagga2 universal:** desde 2026-06-11 ambos escáneres (`admin-scanner.js`, `pos-checkout.js`) usan Quagga2 para todos los dispositivos (antes: rama `isIOS` con html5-qrcode para Android/desktop). La detección `isIOS` del escáner fue eliminada por completo — ya no existe en el código.
