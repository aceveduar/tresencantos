...
- **Comprobante de pago (abono / liquidar saldo) (2026-08-21)** — antes registrar un abono solo mostraba un toast con un botón opcional "Avisar" (fácil de perder) y liquidar el saldo final no ofrecía ninguna forma de avisar a la clienta; ninguno dejaba rastro del lado de ella si la cajera no mandaba el mensaje aparte — causó un reclamo real ("dije que pagué" / "nunca vino") sin forma de verificarlo. Nuevo modal `#abono-done-overlay` (mismo patrón visual que el modal post-venta, reutiliza sus clases CSS) aparece tras `confirmAbonar()`/`confirmLiquidar()`, con "Enviar comprobante por WhatsApp" como acción principal. El mensaje (`sendPaymentReceipt()`, pos-apartados.js) es un recibo fechado — fecha, hora, monto, método, saldo pendiente, folio — WhatsApp le pone su propio timestamp del lado de la clienta, sirviendo de comprobante digital mientras Ofelia no tenga impresora. Si el apartado no tiene teléfono capturado, el modal muestra un campo para capturarlo ahí mismo (solo para ese envío, no se guarda en el apartado). Cerrar el modal sin haber enviado pregunta una vez "¿Cerrar sin enviarle el comprobante?" — recordatorio suave, no bloqueo. Ambos desenlaces quedan auditables en Actividad: `comprobante_enviado` / `comprobante_omitido` (tipo `apartado`), aparte del `apartado_abono`/`apartado_liquidado` que ya registraba el pago en sí — así Ofelia/Eduardo pueden ver, ante un reclamo, si el comprobante realmente se mandó.
