...
- **Auditoría UX/UI "premium/minimalista" del Inventario mobile (2026-08-20)** — 3 hallazgos revisados sobre captura real, 2 implementados:
  - **Chips de estadísticas — tinte rojo persistente para los 3 "urgentes"** (Sin stock, Por caducar, Por revisar): antes los 12 chips se veían con el mismo peso visual (blanco plano) sin importar su `activeColor` semántico (`AR_C_RED`/`AR_C_GREEN`/`AR_C_AMBER`/`AR_C_NEUTRAL`, ya definido por chip desde antes) — el color solo aparecía al activarlos como filtro. `chip()` (`admin-render.js`) ahora aplica un tinte sutil (`${color}0d` fondo, `${color}33` borde, texto sólido) permanentemente cuando `activeColor === AR_C_RED`, para que el ojo detecte problemas sin tener que leer cada número — mismo principio de "resaltar excepciones" que Reportes ya usa con los deltas de KPIs. Los demás chips (verde/ámbar/neutral) no cambian salvo cuando el usuario los activa como filtro.
