...
- **Avisos de sistema — descartables, solo staff (2026-08-21)** — mecanismo nuevo para comunicar cambios del sistema (como el del acceso staff reubicado arriba) sin depender de avisar por fuera. `#sys-notice` (`index.html`, banner dorado fijo justo debajo del header) + `SYS_NOTICES` (array en `app.js`, `{id, text}`) + `initSysNotice()`/`dismissSysNotice()`. Solo se muestra si `document.body` ya tiene la clase `admin-bar-shown` (sesión activa) — nunca a clientas. Se descarta por separado por `id` en `localStorage` (`te_notice_{id}`), así que un aviso nuevo agregado al array siempre aparece aunque uno viejo ya se haya cerrado; solo se muestra uno a la vez (el primero sin descartar). `body.admin-bar-shown.sys-notice-shown .header{top:96px}` empuja el header hacia abajo mientras el aviso está visible, apilado justo debajo de `#admin-bar` (54px) — mismo patrón que ya usa `body.admin-bar-shown .header{top:54px}` para el admin-bar solo. Primer aviso cargado: `staff-access-2026-08`, avisando del cambio de ubicación del acceso a Caja/Inventario. Para agregar un aviso futuro: nuevo objeto `{id, text}` en `SYS_NOTICES`, con un `id` nuevo. CACHE_VERSION v284→v285.
