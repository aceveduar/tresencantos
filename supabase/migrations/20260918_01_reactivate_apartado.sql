-- =============================================================================
-- Reactivar un apartado cancelado por error (2026-09-18)
--
-- Caso real: una cajera cancelo por error un apartado con $1,275 pagados. La
-- cancelacion (cancel_sale_atomic) hizo tres cosas: registro una devolucion
-- en el libro (sale_payments, kind='refund'), regreso los productos a stock
-- y marco el apartado como 'cancelado'. Esta RPC las deshace las tres SIN
-- borrar ni editar historia:
--
--   1. Libro append-only: por cada fila de devolucion de la ULTIMA cancelacion
--      agrega una fila kind='adjustment', monto positivo, mismo metodo y
--      MISMA cajera (collected_by/collected_by_email) que la devolucion, con
--      source='rpc_apartado_reactivation'. Asi el efectivo esperado del turno
--      de quien cancelo se compensa solo (te_close_cash_shift suma
--      sale_payments por collected_by_email), sin inventar dinero en el turno
--      de quien reactiva. La devolucion original NO se toca: el historial
--      queda "pagos, devolucion, reactivacion".
--   2. Stock: vuelve a descontarlo. Si algun producto ya se vendio/aparto a
--      alguien mas, se rechaza TODO (nada queda a medias) y el mensaje dice
--      cuales productos faltan.
--   3. sales: status vuelve a 'activo' (o 'liquidado' si lo pagado cubre el
--      total), cancelled_at NULL, paid_amount = lo que suma el libro.
--
-- Solo superadmin (get_user_role() = 'superadmin'). Deliberadamente NO se
-- puede autorizar con PIN de gerente: reactivar cambia dinero y stock de un
-- registro ya cerrado, es una decision de dueno, no de encargado.
--
-- Idempotente por p_request_id (mismo patron que el resto de RPC de Caja v2)
-- y con control de version optimista (p_expected_version).
--
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.sale_payments (
      ...
      p_sale_id, p_request_id, v_line, 'adjustment', -r.amount, r.method, v_now,
