-- -----------------------------------------------------------------------------
-- edit_apartado_atomic -- agrega snapshot "antes" al log de apartado_editado.
--
-- Motivo: "apartado_editado" solo mostraba el estado nuevo (nuevo total,
-- nueva fecha, etc.) -- Eduardo, viendo el feed de Actividad, no podia saber
-- QUE cambio realmente (¿se quito un producto? ¿bajo el descuento? ¿se movio
-- la fecha limite?), solo que "alguien edito el apartado". v_sale (la fila
-- bloqueada con FOR UPDATE, leida antes del UPDATE) ya tiene todo el estado
-- previo disponible en memoria -- este cambio unicamente agrega esos valores
-- al meta bajo *Previo, junto a los que el log ya guardaba (total, discount,
-- due_date, customer, itemsDetail = todos ya son el valor NUEVO). La UI
-- (_actPopup en activity.js) arma el "Antes -> Despues" comparando ambos
-- lados; no hay logica nueva en la transaccion, solo mas datos en el meta.
--
-- Sin cambios de comportamiento, permisos, validaciones ni firma -- misma
-- funcion que 20260901_03_edit_apartado_phone_activity.sql, un solo bloque
-- jsonb_build_object ampliado.
-- -----------------------------------------------------------------------------
