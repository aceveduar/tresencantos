-- -----------------------------------------------------------------------------
-- edit_apartado_atomic gana p_phone -- permite agregar o corregir el
-- telefono del cliente al editar un apartado (antes solo se podia capturar
-- una vez, al crearlo; si se omitia, no habia forma de agregarlo despues
-- salvo SQL a mano). Motivo real: sin telefono guardado, "Reenviar
-- comprobante" (pos-ui.js resendReceipt/resendLegacyAbono) no tiene a quien
-- mandarle el WhatsApp y pide el numero cada vez, en vez de una sola vez.
--
-- Alcance deliberadamente acotado: solo reescribe el texto libre
-- sales.customer ("Nombre · 📱 Tel"), igual que hace hoy record_sale_atomic_v2
-- al crear la venta. NO toca sales.customer_id ni crea/enlaza un registro en
-- customers -- esa parte de la decision "customer_id inmutable post-creacion"
-- (ver CLAUDE.md, seccion Clientes) se mantiene; aqui solo se resuelve la
-- necesidad puntual de poder enviar el comprobante.
--
-- p_phone (default NULL) = no tocar el telefono actual.
-- p_phone = ''            = quitar el telefono guardado.
-- p_phone = 10 digitos    = fijar/reemplazar el telefono.
-- Cualquier otro valor de p_phone se rechaza (mismo criterio de validacion
-- que record_sale_atomic_v2 usa para pos-checkout.js).
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -- nunca se toca aqui (sigue siendo inmutable post-creacion).
  ...
  PERFORM public.te_log_activity(
    ...
    jsonb_build_object(
      ...
      'phone_updated', (p_phone IS NOT NULL), 'request_id', p_request_id
    )
  );
  IF v_is_final THEN
    PERFORM public.te_log_activity(
