-- =============================================================================
-- Editar apartado: poder corregir el nombre (2026-09-18)
--
-- Hasta ahora el nombre de un apartado era inmutable despues de crearlo (solo
-- se podian ajustar productos, telefono y fecha limite). Un nombre mal
-- capturado -- o una misma clienta capturada como "Lalo" un dia y "Eduardo"
-- otro -- no tenia arreglo. edit_apartado_atomic gana p_name:
--
--   NULL          = no tocar (default; conserva el comportamiento de siempre)
--   texto no vacio = nuevo nombre del apartado (se recorta con btrim, max 80)
--
-- Alcance deliberado, igual que el telefono: solo reescribe el texto
-- sales.customer ("Nombre · 📱 Tel"). NO toca sales.customer_id ni el registro
-- de customers (su nombre se corrige aparte, en Reportes -> Clientes
-- frecuentes). El nombre de un apartado puede ser una descripcion a proposito
-- ("Hija de Marlen calle torres") -- por eso no se sincroniza solo con el
-- cliente.
--
-- Actividad: el summary anexa " · nombre corregido (antes: X)" y el meta
-- guarda name_updated/namePrevio; customerPrevio ya conservaba el texto
-- completo anterior.
--
-- Sin otros cambios de comportamiento, permisos ni validaciones: es la funcion
-- de 20260903_02_edit_apartado_diff.sql con un parametro nuevo al final. Como
-- cambia la lista de parametros, CREATE OR REPLACE crearia un overload nuevo
-- en vez de reemplazar; se hace DROP de la firma anterior de 8 parametros
-- para no dejar dos funciones ambiguas para PostgREST. Un cliente viejo que
-- no manda p_name sigue funcionando (default NULL).
-- =============================================================================
BEGIN;
...
BEGIN
  ...
  -- Nombre y telefono viven juntos en sales.customer ("Nombre · 📱 Tel").
  --   p_phone: NULL = no tocar; '' = quitarlo; 10 digitos = fijarlo.
  --   p_name : NULL = no tocar; texto no vacio = nuevo nombre.
  -- Si no cambia ninguno se conserva v_sale.customer tal cual (mismo
  -- comportamiento de siempre). v_phone_note / v_name_note se anexan al
  -- summary de Actividad para que el cambio no quede invisible.
  v_customer_name := COALESCE(NULLIF(split_part(v_sale.customer, ' · 📱 ', 1), ''), 'Cliente');
  v_customer_tel  := NULLIF(split_part(v_sale.customer, ' · 📱 ', 2), '');
  v_new_name := v_customer_name;
  v_new_tel  := v_customer_tel;

  IF p_name IS NOT NULL THEN
    v_new_name := btrim(p_name);
    IF v_new_name = '' THEN
      RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'El nombre no puede quedar vacío';
    END IF;
    IF char_length(v_new_name) > 80 THEN
      RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'El nombre es demasiado largo (máximo 80 caracteres)';
    END IF;
    IF v_new_name <> v_customer_name THEN
      v_name_note := format(' · nombre corregido (antes: %s)', v_customer_name);
    END IF;
  END IF;
  ...
  ELSE
    v_new_customer := v_new_name || CASE WHEN v_new_tel IS NOT NULL THEN ' · 📱 ' || v_new_tel ELSE '' END;
  END IF;
  ...
  PERFORM public.te_log_activity(
    'apartado_editado',
    format('Edito apartado de %s — nuevo total $%s%s%s%s', v_customer, v_total,
           CASE WHEN v_price_overridden THEN ' (precio modificado)' ELSE '' END,
           v_phone_note, v_name_note),
    jsonb_build_object(
      ...
      'name_updated', (v_name_note <> ''), 'namePrevio', v_customer_name,
      'request_id', p_request_id,
      'itemsDetailPrevio', v_sale.items, 'totalPrevio', v_sale.total,
      'discountPrevio', v_sale.discount, 'dueDatePrevio', v_sale.due_date,
      'customerPrevio', v_sale.customer
    )
  );
  IF v_is_final THEN
    PERFORM public.te_log_activity(
...
-- Firma anterior (8 parametros): se retira para que no convivan dos funciones
-- que ambas aceptan una llamada sin p_name.
DROP FUNCTION IF EXISTS public.edit_apartado_atomic(uuid, bigint, jsonb, bigint, numeric, date, uuid[], text);

REVOKE ALL ON FUNCTION public.edit_apartado_atomic(uuid, bigint, jsonb, bigint, numeric, date, uuid[], text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.edit_apartado_atomic(uuid, bigint, jsonb, bigint, numeric, date, uuid[], text, text) TO authenticated;
